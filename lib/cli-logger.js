...
let logToFile = (file, setUp) => new Promise(function(resolve) {
  let log = function(obj) {
    fs.appendFile(file, JSON.stringify(obj, null, 0) + "\n",
      err => { if (err) throw err; });
  };

  ProcessTree(logTools.GenericLoggerFactory(log), setUp).then(resolve,
    function(error) { log([Date.now, "fatal", error]); resolve(); });
});
