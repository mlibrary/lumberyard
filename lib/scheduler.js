...
module.exports = function(parameters) {
  ...
  if (typeof parameters.tick === "undefined")
    internal.tick = n => new Promise(resolve => {
  ...
  return function(taskObject) {
      ...
      findAll(idleTasks).then(results => {
        ...
        for (let keypair of results)
          if (keypair[1].length > 0)
            running.push(runTask(keypair[0]));

        Promise.all(running).then(resolve, reject);
    ...
    let runTask = id => new Promise(function(resolve, reject) {
