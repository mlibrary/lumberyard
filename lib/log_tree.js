...
tree_factory = function(input_object) {
  ...
  tree.complete = function(input_array) {
    ...
    tree.mark_true(message, message.address);
  };

  tree.mark_true = function(message, address_path) {
    if (address_path.length === 0) {
      if (message.code === "done")
        is_complete = true;

    } else {
      children[address_path.shift()].mark_true(message, address_path);
    }
  };
...
log_message = function(input_array) {
  var address_path, result;

  address_path = [...input_array];
  result = {
    get address() { return [...address_path];}
  };

  result.timestamp = address_path.shift();
  result.code = address_path.shift();
  result.message = address_path.pop();
